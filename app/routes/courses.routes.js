module.exports = (app) => {
    ...
    // Post Courses
    router.post("/courses", CoursesUpload.single("image"), async (req, res) => {
        if (!req.file) {
            return res
                .status(400)
        ...
        try {
            ...
            const outputPath = path.join("./app/public/images/courses", filename);
    ...
    router.put("/course/:id", CoursesUpload.single("image"), async function (req, res, next) {
        try {
          if (!req.file) {
          ...
          const output = path.join("./app/public/images/courses", filename);
          ...
          const updatedCountry = await Courses.findByIdAndUpdate(id, updatedData, { new: true });
      ...
      router.delete("/course/:id", async (req,res)=>{
        try {
            const id = req.params.id;
            const deletedCountry = await Courses.findByIdAndDelete(id, {
