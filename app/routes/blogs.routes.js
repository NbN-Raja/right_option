module.exports = (app) => {
  ...
  router.get("/blog", async (req, res) => {
    try {
      ...
      if (blogs.length === 0) {
        return res
          .status(301)
          .json({ message: "No data in database! Please Add Data" });
      }
      return res
        .status(201)
        .json({ message: "Getting All data from database", data: blogs });
    } catch (error) {
      console.error();
  ...
  router.post("/blog", imageUpload.fields(imageFields), async (req, res) => {
    ...
    try {
      ...
      const {
        image: [imageFile],
        banner_image: [BannerFie],
      } = req.files;
      const [imageName, BannerName] = ["image", "banner_image"].map(
        (prefix) =>
          `${prefix}-${new Date()
            .toISOString()
            .replace(/:/g, "-")}${path.extname(
              req.files[prefix][0].originalname
            )}`
      );
      const [ImageOutput, BannerOutput] = ["image", "banner_image"].map(
        (prefix) =>
          path.join(
            "./public/images/blog",
            `${prefix}-${new Date()
      ...
      res
        .status(200)
        .json({ message: "Blog saved successfully", data: result });
    } catch (error) {
  ...
  router.get("/blog/:id", async (req, res) => {
    try {
      ...
      if (!result) {
        return res
          .status(301)
          .json({ message: "No Blogs found related to this id" });
      }
